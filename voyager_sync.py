...
if __name__ == "__main__":
    ...
    #Parse args to find which clients and how many
    #try with 2 for now.
    # Get an entry for each client expecting to be synced/monitored
    # clientDesc { hostname/IP, portNum }
    numClients = 2
    for i in  0, numClients:
        client = VoyagerClient( clientDesc[i].hostname, clientDesc[i].portNum )
        clients[1].start()
        #assume they all start ok or move to try block
        clients.add( client )
  
#   client [0] is the master and assumed connected and running.
#    clients[1].add_handler('NewJPGReady', print_jpg_info)

    clients[1].cmd.set_logs('enable')
    clients[1].cmd.set_dashboard('enable')
